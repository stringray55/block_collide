...
class Block:
    ...
    def frame_energy(self,f):
        ...
        self.corners=self.get_returned_corners(self.centered_corners)
        self.block_ax=self.fig.add_subplot(self.grid[:4,:4],projection='3d',xlim=(-50,50),ylim=(-50,50),zlim=(0,100))
        if self.t<(2/3)*self.t_plot_range:
            self.energy_ax=self.fig.add_subplot(self.grid[4:6,:4],xlim=(0,self.t_plot_range))
        else:
            self.energy_ax=self.fig.add_subplot(self.grid[4:6,:4],xlim=(self.t-(2/3)*self.t_plot_range,self.t+(1/3)*self.t_plot_range))
    ...
    def animate_block_energy(self,save_name,size,dt,tf,t_plot_range,steps_per_frame,camera_rot):
        self.t=0
        self.dt=dt
        self.tf=tf
        self.t_past=False
        self.t_plot_range=t_plot_range
        self.steps_per_frame=steps_per_frame
        self.camera_rot=camera_rot
